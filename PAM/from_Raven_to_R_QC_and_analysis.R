...
##### This is a demonstration of the process and explanation of the steps. 
####  The quality of the results and the outputs is very much dependent on 
##### the quality of the audio material and the consistency of the call annotation process.
##### for using the script you can use the provided example files 

#### WHAT THE SCRIPT DOES? 
   # 1. Loads selection tables generated in RAVEN PRO
   # 2. Links the annotated selections to the Audio Files
   # 3. Goes through several simple quality control steps
   #    a. Visual checks for consistency of data
   #    b. Measure Signal To Noise ratio for each sound
   #    c. Calculate spectral cross correlation between sounds of the same type
   #    d. Attempt to trace dominant frequency to make sure we can measure stuff
   # 4. Suggest few usefull functions for measuring spectral and temporal features of the sound
...
#look at the selection files in the folder, just for sanity check to make sure 
#don't have any random garbahe files
...
#### WARNING #### to makes the script run on your machine with the provided example files
                # the folder path in the 'sound.file.col' needs to be changed
                # when creating new selection tables locally this step can be skipped
  

# replace the path to the wave file with your own
rvn.dat$`Begin Path`<-paste(getwd(),rvn.dat$sound.files, sep = "/")
...
## We are done with data pre-processing and can do some quick data quality checks ##
...
# we also add some buffer time (0.1s) before and after the call to avoid having very short audio segments 
# the duration of the buffer time can be adjusted according to your data
wv1 <- read_wave(X = rvn.dat.st$`Begin Path`[r] , from = rvn.dat.st$start[r]-0.1, to = rvn.dat.st$end[r]+0.1)

#play the audio - this command might be different for mac 
#or might not work at all if the default WAV player on your computer 
#was changed. If it is not working or throwing errors GOOGLE the relevant solution
#according to your operating system or just comment this step out
#it is not effecting any of the subsequent analyses
play(wv1)

#plot a spectrogram of the current sound segment
#adjust spectrogram parameters as needed. Spevtrogram settings
#depend on the duration and frequency of sounds that you work with
...
#reasonably consistent and of a good enough quality
...
#if you have a lot of calls you might need to subset and display a random selection here
#the goal of this step is to eyeball the data and to make sure it is reasonably consistent
...
#in our example data the SNR values are all very high so we can move on
